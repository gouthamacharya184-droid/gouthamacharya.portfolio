...
import { contactSchema, safeErrorMessage } from "../config/validation.js";
...
export const handleContact = async (req, res) => {
  ...
  try {
    ...
    logger.info({ type: "contact_sent", email: result.data.email.replace(/(.{2}).+(@.+)/, "$1***$2") });
    return res.status(200).json({
      ok:      true,
      message: "Your message was sent successfully. I'll get back to you soon!",
    });
  } catch (error) {
    logger.error({ type: "contact_failed", msg: error.message });
    return res.status(500).json({
      ok:      false,
      message: safeErrorMessage("Message could not be sent right now. Please try again later."),
    });
  }
};
